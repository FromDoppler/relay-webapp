(function(helpers) {
    ...
    function adapterService($http, $q) {
      ...
      function getPreviewImageWithPlay(params) {
        helpers().logger("getPreviewImageWithPlay", arguments);
        var response = {
      ...
      function getPreviewImage(params) {
        helpers().logger("getPreviewImage", arguments);
        var response = {
      ...
      function getImagesForCampaign(start, howMany, query, sortingCriteria) {
        helpers().logger("getImagesForCampaign", arguments);
        var images = [{
          name: 'fakeImage1.jpg',
      ...
      function deleteImages(filenames) {
        helpers().logger("deleteImages", arguments);
        var response = {
          success: true,
          error: false
        }
      ...
      function getCampaignRssPreview(idCampaign, subjectCampaign, html) {
        helpers().logger("getCampaignRssPreview", arguments);
        var rssTags = [
          {
      ...
      function campaignSaveChangesAsTemplate(params) {
        helpers().logger("campaignSaveChangesAsTemplate", arguments);
      ...
      function campaignSaveChanges(params) {
        helpers().logger("campaignSaveChanges", arguments);
        var savedCampaign = {
          data: {
            Success: true,
            ErrorMessage: '',
      ...
      function getCampaign(id, useEditorAsTemplate) {
        helpers().logger("getCampaign", arguments);
        var campaign = {
          id: id,
          type: 'campaign',
          name: 'Campaign ' + id,
      ...
      function getSettings(idCampaign, idTemplate) {
        helpers().logger("getSettings", arguments);
      ...
      */
      ...
      function refreshMercadoShopsProductServiceToken() {
        helpers().logger("refreshMercadoShopsProductServiceToken", arguments);
      ...
      function getTiendaNubeProducts(storeId, accessToken, page, limit, sortBy, query) {
        helpers().logger("getTiendaNubeProducts", arguments);
        return $q.resolve({});
      }
    }
  })(helpers);
  ...
  function logger(name, args) {
    console.log(name, Array.from(args));
  }